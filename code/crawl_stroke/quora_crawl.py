...
def crawlTopicHierarchy():
    ...
    file_topic_names = open("../../resource/stroke/topic_names.txt", mode = 'w')
    file_topic_urls = open("../../resource/stroke/topic_urls.txt", mode = 'w')

    # Starting node link
    # url = 'http://www.quora.com/Preventive-Medicine?share=1'
    url = 'https://www.quora.com/topic/Strokes-medical-condition?share=1'
...
def crawlTopicQuestions(topic_urls):
    ...
    file_question_urls =  open("../../resource/stroke/question_urls.txt", mode = 'w')
    file_topic_urls = open("../../resource/stroke/topic_urls.txt", mode = 'r')
    ...
    for topic in range(len(topic_urls)):
        ...
        # Fetch current page
        fw = open("../../resource/stroke/page", mode = 'w')
        src_updated = browser.page_source
        src = ""
        while src != src_updated:
            time.sleep(.5)
        ...
        fw.close()
        # if '<h3>' in html_source:
            # print('='*50)

        browser.quit()

        string_for_split = '<div class="CssComponent-sc-1oskqb9-0 cXjXFI">'

        split_html = html_source.split(string_for_split)
...
def crawlQuestionData(file):
    ...
    file_data = open("../../resource/stroke/answers.csv", mode = 'w')
    file_users = open("../../resource/stroke/users.txt", mode = 'w')
    ...
    while (current_line):
        ...
        # Find question text
        string_for_split_question = '<span style="background: none;">'

        question_text = html_source.split(string_for_split_question)[1]
...
def crawlUser():
    ...
    unique_users = set(open("../../resource/stroke/users.txt").readlines())
    bar = open('../../resource/stroke/temp.txt', 'w').writelines(set(unique_users))
    
    file_users = open("../../resource/stroke/temp.txt", mode='r')
    file_users_csv = open("../../resource/stroke/users.csv", mode='w')
    total = 0
...
def main():
    ...
    crawlQuestionData("../../resource/stroke/question_urls.txt")
    crawlUser();
    return 0
